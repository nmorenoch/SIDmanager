...
class dataFile():
   ...
   def __init__(self, oFile, dimension, lbox, rho, polyL, ro, pFrac, blockFracs, nSolv, solvFrac, additBF=None, addFrac=0.0, addLen=1, location=0,lcore=1):
      ...
      #if lbox[0] == lbox[1]:
      self.lbox = np.array(lbox.split(','), dtype=float)
      ...
      self.polyL = polyL
      self.ro = ro
      print('bond distance set on %s' % ro)
      self.pFrac = pFrac
      ...
      self.nPart        = int(self.lbox[0]*self.lbox[1]*self.lbox[2]*self.rho)
      ...
      if self.location==5: ###location of the whole box centered in 0
          self.xlo, self.xhi = -self.lbox[0]/2., self.lbox[0]/2.
          self.ylo, self.yhi = -self.lbox[1]/2., self.lbox[1]/2.
          self.zlo, self.zhi = -self.lbox[2]/2.*self.eFact*(self.dimension-2), self.lbox[2]/2.*self.eFact*(self.dimension-2)
      else:
          self.xlo, self.xhi = 0, self.lbox[0]
          self.ylo, self.yhi = 0, self.lbox[1]
          self.zlo, self.zhi = 0, self.lbox[2]*self.eFact*(self.dimension-2)
      ...
      ##If polymer
      if self.location==1:
         self.polyLimits = self.zlo+self.lbox[2]*self.eFact*(self.solvFrac[0])#+self.pFrac+self.addFrac)
      else:
   ...
   def additive(self, f, preK):
       ...
       if self.location==4:
          ...
          seed = (np.random.rand(self.addNChains,3)-0.5)*(self.lcore)+self.lbox[1]/2
   ...
   def atoms(self):
       ...
       if self.location==1: ##Polymer localized with one fluid 
          seedx  = np.random.rand(self.nChains,1)*(self.lbox[0]*self.eFact*(self.pFrac+self.addFrac))-self.zhi
          seedy = np.random.rand(self.nChains,2)*self.lbox[1]-self.lbox[1]/2
          ...
          seedz  = np.random.rand(self.nChains,1)*(self.lbox[2]*self.eFact)-self.zhi
          seedx = np.random.rand(self.nChains,1)*self.lbox[0]-self.lbox[0]/2
          seedy = np.random.rand(self.nChains,1)*self.lbox[1]-self.lbox[1]/2

          seed = np.append(seedx,seedy,1) ##The first row correspond to the x coordinates
          seed = np.append(seed,seedz,1) ##append z coordinate for consistency
          ...
          seed = (np.random.rand(self.nChains,3)-0.5)*(self.lcore)+self.lbox[1]/2
       elif self.location==7: ##Polymer localized at the center of the box in a cube equivalent to it fraction.
          ...
          print ("located slab")
          lcube = self.lbox*self.e1Fact*(self.pFrac+self.addFrac)**(1./self.dimension)
...
#          xc, yc, zc = self.randomXYZarrayAtLocation(self.solventBeads)
       ...
       a, t,m = 1,1,1
       if self.nChains>0:  ##only update if polymer chains exist.
   ...
   def randomXYZarrayAtLevels(self, randomPoints):
        ...
        lcube = self.lbox[2]*self.eFact*(self.pFrac+self.addFrac)**(1./3.)
        gapz  = lcube*1./self.location
        
        z1 = np.random.rand(beadS1)*(self.lbox[2]*self.eFact*frOfBox)-(self.zhi+gapz)
        z2 = gapz+self.zhi-np.random.rand(beadS2)*(self.lbox[2]*self.eFact*(1-frOfBox))#(1-self.pFrac+self.addFrac)-self.polyLimits
                                                                                    #        x = np.random.rand(randomPoints)*(self.lbox*(1-self.pFrac+self.addFrac))-self.lbox/2
        print (self.polyLimits)
